...
def add_suffix(obj):
    if consts.EXPORT_TEMP_SUFFIX not in obj.name:
        newname = obj.name + consts.EXPORT_TEMP_SUFFIX
        print("Add Suffix (Object name): [" + obj.name + "] -> [" + newname + "]")
        obj.name = newname

    # インスタンス化されたメッシュがあるとインスタンスの個数分だけ関数が呼ばれるため、suffixが多重追加されないように対策しておく
    if obj.data is not None and not consts.EXPORT_TEMP_SUFFIX in obj.data.name:
        newname = obj.data.name + consts.EXPORT_TEMP_SUFFIX
...
def remove_suffix(obj):
    if consts.EXPORT_TEMP_SUFFIX in obj.name:
        oldname = obj.name
        newname = oldname[0:oldname.rfind(consts.EXPORT_TEMP_SUFFIX)]
