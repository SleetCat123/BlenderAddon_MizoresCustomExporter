...
def validate_after_complete():
    try:
        t.validate_exported_outputs(PENDING_EXPECTED_FILES)
...
def patch_export_operator(expected_files):
    global PENDING_EXPECTED_FILES
    PENDING_EXPECTED_FILES = expected_files
    ...
    def _complete(self, context):
        try:
            ...
            bpy.app.timers.register(validate_after_complete, first_interval=0.1)
...
def setup_and_export():
    try:
        expected_files = t.build_multi_armature_export_set_scene()
